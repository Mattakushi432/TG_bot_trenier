# API Guide - IFBB Pro Dual-Coach AI

## Получение API ключей

### 1. Telegram Bot Token

1. Откройте Telegram и найдите [@BotFather](https://t.me/BotFather)
2. Отправьте команду `/newbot`
3. Следуйте инструкциям:
   - Введите имя бота (например: "IFBB Pro Coach")
   - Введите username бота (должен заканчиваться на "bot", например: "ifbb_pro_coach_bot")
4. Скопируйте полученный токен
5. Вставьте токен в файл `.env`:
   ```
   TELEGRAM_BOT_TOKEN=1234567890:ABCdefGHIjklMNOpqrsTUVwxyz
   ```

### 2. Google Gemini API Key

1. Перейдите в [Google AI Studio](https://makersuite.google.com/app/apikey)
2. Войдите в аккаунт Google
3. Нажмите "Create API Key"
4. Выберите проект или создайте новый
5. Скопируйте созданный ключ
6. Вставьте ключ в файл `.env`:
   ```
   GEMINI_API_KEY=AIzaSyABC123def456GHI789jkl
   ```

## Настройка бота в Telegram

### Базовые команды BotFather

```
/setname - Изменить имя бота
/setdescription - Установить описание
/setabouttext - Установить текст "О боте"
/setuserpic - Загрузить аватар бота
/setcommands - Настроить меню команд
```

### Рекомендуемые настройки

**Имя бота:**
```
IFBB Pro Dual-Coach AI
```

**Описание:**
```
🏆 Персональный ИИ-тренер с двойной личностью: Ронни Коулман для мужчин, Дженет Лайог для женщин. Индивидуальные программы тренировок, планы питания и спортивное питание от новичка до уровня Olympia.
```

**О боте:**
```
🏋️ Что я умею:
• Создаю персональные планы тренировок на 4 недели
• Рассчитываю точное КБЖУ и план питания
• Подбираю спортивное питание
• Отслеживаю прогресс и адаптирую программы
• Работаю как Ронни Коулман (для мужчин) или Дженет Лайог (для женщин)

🎯 Цели: от подтянутого тела до подготовки к соревнованиям
📍 Локации: зал или дом
🧬 Научный подход: формулы BMR/TDEE, периодизация, прогрессия

Начни с команды /start
```

**Команды меню:**
```
start - Начать работу с ботом
help - Справка по функциям
reset - Сбросить данные профиля
```

## Лимиты и ограничения API

### Telegram Bot API

- **Сообщения**: До 30 сообщений в секунду
- **Размер сообщения**: До 4096 символов
- **Файлы**: До 50 МБ
- **Inline клавиатуры**: До 100 кнопок

### Google Gemini API

- **Бесплатный тариф**: 
  - 60 запросов в минуту
  - 1500 запросов в день
- **Размер промпта**: До 30,720 символов
- **Размер ответа**: До 2048 токенов

## Мониторинг использования

### Telegram

Статистика доступна через [@BotFather]:
```
/mybots → Выберите бота → Bot Settings → Statistics
```

### Google AI Studio

Мониторинг в [Google AI Studio](https://makersuite.google.com/app/apikey):
- Количество запросов
- Использованные токены
- Ошибки API

## Обработка ошибок API

### Типичные ошибки Telegram

```python
# Превышение лимита символов
if len(message) > 4096:
    # Разбить на части или сократить

# Слишком много запросов
except telegram.error.RetryAfter as e:
    time.sleep(e.retry_after)

# Бот заблокирован пользователем
except telegram.error.Forbidden:
    # Удалить пользователя из активных
```

### Типичные ошибки Gemini

```python
# Превышение лимита запросов
except google.api_core.exceptions.ResourceExhausted:
    # Подождать или использовать fallback

# Неподходящий контент
except google.api_core.exceptions.InvalidArgument:
    # Изменить промпт или использовать заглушку

# Проблемы с сетью
except google.api_core.exceptions.ServiceUnavailable:
    # Повторить запрос с задержкой
```

## Оптимизация производительности

### Кэширование ответов

```python
# Кэш для часто запрашиваемых данных
response_cache = {}

def get_cached_response(user_id, query_type):
    cache_key = f"{user_id}_{query_type}"
    if cache_key in response_cache:
        return response_cache[cache_key]
    return None
```

### Батчинг запросов

```python
# Группировка запросов к базе данных
def batch_update_users(user_updates):
    with sqlite3.connect(DATABASE_PATH) as conn:
        cursor = conn.cursor()
        cursor.executemany(
            "UPDATE users SET weight = ? WHERE user_id = ?",
            user_updates
        )
        conn.commit()
```

### Асинхронная обработка

```python
# Неблокирующие операции
async def handle_long_operation(update, context):
    await update.message.reply_text("⏳ Обрабатываю...")
    
    # Длительная операция в фоне
    result = await asyncio.create_task(
        generate_workout_plan(user_data)
    )
    
    await update.message.reply_text(result)
```

## Безопасность

### Защита API ключей

```bash
# Никогда не коммитьте .env файлы
echo ".env" >> .gitignore

# Используйте переменные окружения в продакшене
export TELEGRAM_BOT_TOKEN="your_token"
export GEMINI_API_KEY="your_key"
```

### Валидация входных данных

```python
def validate_user_input(data):
    # Проверка возраста
    if not (16 <= data.get('age', 0) <= 80):
        raise ValueError("Некорректный возраст")
    
    # Проверка веса
    if not (40 <= data.get('weight', 0) <= 200):
        raise ValueError("Некорректный вес")
    
    # Санитизация текста
    data['injuries'] = html.escape(data.get('injuries', ''))
```

### Ограничение частоты запросов

```python
from collections import defaultdict
import time

user_last_request = defaultdict(float)
REQUEST_COOLDOWN = 2  # секунды

def check_rate_limit(user_id):
    now = time.time()
    if now - user_last_request[user_id] < REQUEST_COOLDOWN:
        return False
    user_last_request[user_id] = now
    return True
```

## Развертывание в продакшене

### Переменные окружения

```bash
# Обязательные
TELEGRAM_BOT_TOKEN=your_token
GEMINI_API_KEY=your_key

# Опциональные
DATABASE_PATH=/app/data/users.db
LOG_LEVEL=INFO
MAX_USERS=10000
CACHE_TTL=3600
```

### Docker развертывание

```dockerfile
FROM python:3.11-slim

WORKDIR /app
COPY requirements.txt .
RUN pip install -r requirements.txt

COPY . .
CMD ["python", "main.py"]
```

### Мониторинг в продакшене

```python
import logging
import sentry_sdk

# Настройка логирования
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler('bot.log'),
        logging.StreamHandler()
    ]
)

# Мониторинг ошибок
sentry_sdk.init(dsn="your_sentry_dsn")
```

## Масштабирование

### Горизонтальное масштабирование

- Используйте webhook вместо polling
- Распределите нагрузку через load balancer
- Вынесите базу данных в отдельный сервис

### Оптимизация базы данных

```sql
-- Индексы для быстрого поиска
CREATE INDEX idx_users_user_id ON users(user_id);
CREATE INDEX idx_progress_user_id ON progress(user_id);
CREATE INDEX idx_progress_date ON progress(date);

-- Партиционирование по датам
CREATE TABLE progress_2024 PARTITION OF progress
FOR VALUES FROM ('2024-01-01') TO ('2025-01-01');
```

### Кэширование

```python
import redis

# Redis для кэширования
redis_client = redis.Redis(host='localhost', port=6379, db=0)

def cache_user_data(user_id, data, ttl=3600):
    redis_client.setex(
        f"user:{user_id}", 
        ttl, 
        json.dumps(data)
    )
```